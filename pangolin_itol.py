...
#think in a more efficient way to parse lineages and colors...
...
annotation_info = open(itol_file+'.pan.info','w',newline='')
writer_out_file = csv.writer(annotation_info)

def func_colors(pangolin_file, itol_file):
    """
    This function execute the match between contries in name sequences and contries in reference file.

    Keyword arguments:
    pangolin_file - csv file with pangolin output, parsed with -in argument.
    itol_file - iTOL template file, parsed with -it argument.
    """
    with open(pangolin_file,'r') as pan_file, open(itol_file,'r') as itol_mod:
